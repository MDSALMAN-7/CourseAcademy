// Here we will show you the h course that costamer purchased
...
const Purchases = () => {
  ...
  const [errorMessage, setErrorMassage] = useState(true);
  ...
  useEffect(() => {
    ...
    const token = user?.token;
     ...
     const fetchPurchases = async () =>{
        if(!token){
            setErrorMassage("Please login to purchase course.");
            return;
        }

        try {
            const response = await axios.get("http://localhost:5001/api/v1/user/purchases",{
                headers:{
           ...
           setErrorMassage( "Faild to fetch purchases data");
        }
    };
    fetchPurchases();
  }, []);
